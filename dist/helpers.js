 ...
 */
function mergeHashes(source, target) {
  var options =
    arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {}
  var resetValues =
  ...
  for (var key in source) {
    ...
    if (hasNestedEntries) {
      ...
      if (
        ...
        (pluralMatch &&
          hasRelatedPluralKey(
            ''.concat(singularKey).concat(pluralSeparator),
            target
          ))
      ) {
        target[key] = source[key]
...
function isPlural(key) {
  return pluralSuffixes.some(function (suffix) {
...
function hasRelatedPluralKey(rawKey, source) {
  return pluralSuffixes.some(function (suffix) {
    return source[''.concat(rawKey).concat(suffix)] !== undefined
  })
}

function getSingularForm(key, pluralSeparator) {
  var pluralRegex = new RegExp(
    '(\\'.concat(pluralSeparator, '(?:zero|one|two|few|many|other))$')
  )
