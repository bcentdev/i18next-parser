...
var JsxLexer = /*#__PURE__*/ (function (_JavascriptLexer) {
  _inherits(JsxLexer, _JavascriptLexer)
  var _super = _createSuper(JsxLexer)
  function JsxLexer() {
    var _this
    var options =
      arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {}
    _classCallCheck(this, JsxLexer)
  ...
  _createClass(JsxLexer, [
        ...
        var parseTree = function parseTree(node) {
          ...
          switch (node.kind) {
            ...
            case ts.SyntaxKind.TaggedTemplateExpression:
              entry = _this2.taggedTemplateExpressionExtractor.call(
                _this2,
                node
              )
              break
    ...
    {
      key: 'jsxExtractor',
      value: function jsxExtractor(node, sourceText) {
        var _this3 = this
        var tagNode = node.openingElement || node

        var getPropValue = function getPropValue(node, tagName) {
          var _attribute$initialize
          var attribute = node.attributes.properties.find(function (attr) {
            return attr.name.text === tagName
          })
          ...
          if (
            ((_attribute$initialize = attribute.initializer.expression) ===
              null || _attribute$initialize === void 0
              ? void 0
              : _attribute$initialize.kind) === ts.SyntaxKind.Identifier
          ) {
            _this3.emit(
              'warning',
              'Namespace is not a string literal: '.concat(
        ...
        var getKey = function getKey(node) {
          return getPropValue(node, _this3.attr)
        }
    ...
    {
      key: 'nodeToString',
      value: function nodeToString(node, sourceText) {
        var _this4 = this
        var children = this.parseChildren.call(this, node.children, sourceText)

        var elemsToString = function elemsToString(children) {
          return children
    ...
    },
    {
      ...
      value: function parseChildren() {
        ...
        var children =
          arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : []
        var sourceText = arguments.length > 1 ? arguments[1] : undefined
        return children
          .map(function (child) {
            if (child.kind === ts.SyntaxKind.JsxText) {
              ...
              if (!child.expression) {
                ...
                child.expression.kind === ts.SyntaxKind.StringLiteral
              ) {
                return {
                  type: 'text',
              ...
              return {
                type: 'js',
                content: '{'.concat(
                  sourceText.slice(child.expression.pos, child.expression.end),
                  '}'
                ),
              }
            } else {
              throw new Error(
                'Unknown ast element when parsing jsx: ' + child.kind
              )
            }
          })
