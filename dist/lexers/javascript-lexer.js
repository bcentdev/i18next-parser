...
var JavascriptLexer = /*#__PURE__*/ (function (_BaseLexer) {
  _inherits(JavascriptLexer, _BaseLexer)
  ...
  _createClass(JavascriptLexer, [
    {
      key: 'createCommentNodeParser',
      value: function createCommentNodeParser() {
        var _this2 = this
    ...
    {
      key: 'setNamespaces',
      value: function setNamespaces(keys) {
        var _this3 = this
    ...
    {
      key: 'setKeyPrefixes',
      value: function setKeyPrefixes(keys) {
        var _this4 = this
        if (this.keyPrefix) {
    ...
    {
      key: 'taggedTemplateExpressionExtractor',
      value: function taggedTemplateExpressionExtractor(node) {
        var entry = {}
    ...
    {
      key: 'expressionExtractor',
      value: function expressionExtractor(node) {
        var _this6 = this
        var entry = {}
        ...
        if (
          ...
          var _node$arguments$ = node.arguments[0],
            text = _node$arguments$.text,
            elements = _node$arguments$.elements
          ...
          if (text) {
            ...
            if (
              ...
              var _node = optionsArgument.properties.find(function (p) {
                return p.name.escapedText === 'keyPrefix'
              })
        ...
        var isTranslationFunction =
          (node.expression.text &&
            this.functions.includes(node.expression.text)) ||
        ...
        if (isTranslationFunction) {
          ...
          if (
            ...
            if (!concatenatedString) {
              this.emit(
                'warning',
                'Key is not a string literal: '.concat(keyArgument.text)
              )
          ...
          if (this.parseGenerics && node.typeArguments) {
            ...
            var parseTypeArgument = function parseTypeArgument(typeArg) {
              ...
              if (typeArg.kind === ts.SyntaxKind.TypeLiteral) {
                var _iterator = _createForOfIteratorHelper(typeArg.members),
                  _step
                try {
                ...
                if (typeName in _this6.typeMap) {
                  Object.assign(entry, _this6.typeMap[typeName])
                }
              } else if (Array.isArray(typeArg.types)) {
                typeArgument.types.forEach(function (tp) {
                  return parseTypeArgument(tp)
                })
              }
            }
            ...
            try {
              for (_iterator2.s(); !(_step2 = _iterator2.n()).done; ) {
                ...
                if (p.kind === ts.SyntaxKind.SpreadAssignment) {
                  this.emit(
                    'warning',
                    'Options argument is a spread operator : '.concat(
                      p.expression.text
                    )
                  )
    ...
    {
      key: 'commentExtractor',
      value: function commentExtractor(commentText) {
        var _this7 = this
        ...
        expressions.forEach(function (expression) {
          var expressionKeys = _this7.extract(expression)
          if (expressionKeys) {
    ...
    },
    {
      ...
      value: function concatenateString(binaryExpression) {
        var string =
        ...
        if (binaryExpression.right.kind === ts.SyntaxKind.BinaryExpression) {
          ...
          binaryExpression.right.kind === ts.SyntaxKind.StringLiteral
        ) {
...
export { JavascriptLexer as default }
