...
var Parser = /*#__PURE__*/ (function (_EventEmitter) {
  ...
  function Parser() {
    ...
    _this.lexers = _objectSpread(_objectSpread({}, lexers), options.lexers)
    return _this
  }
  _createClass(Parser, [
    {
      key: 'parse',
      value: function parse(content, filename) {
        var _this2 = this
        var keys = []
        var extension = path.extname(filename).substr(1)
        var lexers = this.lexers[extension] || this.lexers['default']
        var _iterator = _createForOfIteratorHelper(lexers),
          _step
        try {
          for (_iterator.s(); !(_step = _iterator.n()).done; ) {
            var lexerConfig = _step.value
            ...
            if (typeof lexerName === 'function') {
              ...
              if (!lexersMap[lexerName]) {
                this.emit(
                  'error',
                  new Error("Lexer '".concat(lexerName, "' does not exist"))
                )
              }
