...
var i18nTransform = /*#__PURE__*/ (function (_Transform) {
  _inherits(i18nTransform, _Transform)
  var _super = _createSuper(i18nTransform)
  function i18nTransform() {
    var _this
    var options =
      arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {}
    _classCallCheck(this, i18nTransform)
    ...
    _this.options.i18nextOptions = _objectSpread(
      _objectSpread({}, options.i18nextOptions),
      {},
      {
    ...
    _this.parser.on('warning', function (warning) {
      return _this.warn(warning)
    })
  ...
  _createClass(i18nTransform, [
    ...
    {
      key: '_transform',
      value: function _transform(file, encoding, done) {
        var content
        if (file.isBuffer()) {
        ...
        try {
          for (_iterator.s(); !(_step = _iterator.n()).done; ) {
            ...
            key = key.replace(/\\\\/g, '\\')
            entry.key = key
            entry.keyWithNamespace =
              entry.namespace + this.options.keySeparator + key
    ...
    },
    {
      ...
      value: function _flush(done) {
        ...
        if (this.options.resetDefaultValueLocale) {
          ...
          maybeSortedLocales.sort(function (a) {
            return a === _this2.options.resetDefaultValueLocale ? -1 : 1
          })
        }

        // Tracks keys to reset by namespace
        var resetValues = {}
        var _iterator2 = _createForOfIteratorHelper(maybeSortedLocales),
          _step2
        try {
          var _loop = function _loop() {
            ...
            var transformEntry = function transformEntry(entry, suffix) {
              ...
              var _dotPathToHash = dotPathToHash(entry, catalog, {
                ...
                duplicate = _dotPathToHash.duplicate,
                conflict = _dotPathToHash.conflict

              if (duplicate) {
                if (conflict === 'key') {
                  _this2.warn(
                    'Found translation key already mapped to a map or parent of ' +
                      'new key already mapped to a string: '.concat(entry.key)
                  )
                } else if (conflict === 'value') {
                  _this2.warn(
                    'Found same keys with different values: '.concat(entry.key)
                  )
                }
              } else {
            ...
            var _iterator3 = _createForOfIteratorHelper(_this2.entries),
              _step3
            try {
            ...
            for (var namespace in catalog) {
              ...
              var namespaceOldPath = path.join(
                ...
                ''
                  .concat(parsedNamespacePath.name, '_old')
                  .concat(parsedNamespacePath.ext)
              )
              ...
              var _mergeHashes = mergeHashes(
                ...
                newCatalog = _mergeHashes['new'],
                oldKeys = _mergeHashes.old,
                mergeCount = _mergeHashes.mergeCount,
                oldCount = _mergeHashes.oldCount,
                resetFlags = _mergeHashes.reset,
                resetCount = _mergeHashes.resetCount
              ...
              var _mergeHashes2 = mergeHashes(
                  ...
                  newCatalog,
                  _objectSpread(
                    _objectSpread({}, _this2.options),
                    {},
                    { keepRemoved: false }
                  )
                ),
                oldCatalog = _mergeHashes2.old,
                restoreCount = _mergeHashes2.mergeCount
              ...
              if (_this2.options.verbose) {
                ...
                console.log(
                  'Unique keys: '
                    .concat(uniqueCount[namespace], ' (')
                    .concat(uniquePluralsCount[namespace], ' are plurals)')
                )
              ...
              if (sort) {
                ...
                maybeSortedOldCatalog = sortKeys(oldCatalog, {
                  deep: true,
                  compare: compare,
                })
              }
    ...
    {
      key: 'getCatalog',
      value: function getCatalog(path) {
        try {
          var content
    ...
    {
      key: 'pushFile',
      value: function pushFile(path, contents) {
        var text
        if (path.endsWith('yml')) {
          text = yaml.dump(
            contents,
          ...
          text = text.replace(
            ...
            function (chr) {
              ...
              return n < 128
                ? chr
                : '\\u'.concat('0000'.concat(n.toString(16)).substr(-4))
            }
          )
        }
  ...
  return i18nTransform
})(Transform)
export { i18nTransform as default }
